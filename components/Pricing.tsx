...
interface PricingProps {
  onSelectPlan?: (plan: 'free' | 'pro' | 'master') => void;
  currentPlan?: 'free' | 'pro' | 'master';
}

const Pricing: React.FC<PricingProps> = ({ onSelectPlan, currentPlan = 'free' }) => {
  const { t } = useLanguage();
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-24 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
        {/* Free Plan */}
        <div className={`bg-white rounded-[3rem] p-10 border flex flex-col transition-all duration-500 ${
          currentPlan === 'free' ? 'border-indigo-600 ring-2 ring-indigo-50 shadow-md' : 'border-slate-100 shadow-sm hover:border-slate-200'
        }`}>
          <div className="mb-10">
            ...
            <div className="flex items-center gap-2">
              ...
              {currentPlan === 'free' && <span className="text-[10px] font-bold bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full uppercase">Active</span>}
            </div>
          </div>
          ...
          <button 
            ...
            className={`w-full py-4 px-6 rounded-[1.2rem] font-bold transition-all active:scale-[0.98] ${
              currentPlan === 'free' ? 'bg-slate-100 text-slate-400 cursor-default' : 'bg-slate-50 text-slate-900 hover:bg-slate-100'
            }`}
          >
            {currentPlan === 'free' ? 'Your Current Rhythm' : t('pricing.plans.free.cta')}
          </button>
        </div>

        {/* Pro Plan */}
        <div className={`rounded-[3rem] p-10 flex flex-col relative overflow-hidden group transition-all duration-700 ${
          currentPlan === 'pro' ? 'bg-indigo-700 shadow-2xl scale-105 ring-4 ring-indigo-100' : 'bg-indigo-600 text-white shadow-xl hover:shadow-2xl'
        }`}>
          ...
          <div className="relative z-10 flex flex-col h-full text-white">
            <div className="mb-10">
              ...
              <div className="flex items-baseline gap-2 flex-wrap">
                ...
                {currentPlan === 'pro' && <span className="text-[10px] font-bold bg-white/20 text-white px-2 py-0.5 rounded-full uppercase">Active</span>}
              </div>
            </div>
            ...
            <button 
              ...
              className={`w-full py-4 px-6 rounded-[1.2rem] font-bold transition-all shadow-lg active:scale-[0.98] ${
                currentPlan === 'pro' ? 'bg-indigo-500/50 text-indigo-200 cursor-default' : 'bg-white text-indigo-600 hover:bg-indigo-50'
              }`}
            >
              {currentPlan === 'pro' ? 'Your Current Rhythm' : t('pricing.plans.pro.cta')}
            </button>
          </div>
        </div>

        {/* Zen Master (Coming Soon) */}
        <div className={`bg-white rounded-[3rem] p-10 border flex flex-col relative overflow-hidden group border-dashed opacity-80 transition-all duration-500 ${
          currentPlan === 'master' ? 'border-violet-500 opacity-100' : 'border-slate-100'
        }`}>
